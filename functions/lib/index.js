...
exports.onRegistrationUpdate = exports.verifyOTP = exports.sendOTP = void 0;
...
exports.sendOTP = (0, https_1.onCall)(async (request) => {
    ...
    if (!email) {
        throw new https_1.HttpsError('invalid-argument', 'The function must be called with an email address.');
    }
    // Regex validation for college emails ending in .ac.in
    const collegeEmailRegex = /^[^\s@]+@[^\s@]+\.ac\.in$/;
    if (!collegeEmailRegex.test(email)) {
        throw new https_1.HttpsError('invalid-argument', 'Only college emails (.ac.in) are permitted.');
    }
...
exports.verifyOTP = (0, https_1.onCall)(async (request) => {
    const { email, otp, fullName, college, isSignup } = request.data;
    if (!email || !otp) {
    ...
    if (!otpDoc.exists) {
        throw new https_1.HttpsError('not-found', 'No verification code found for this email.');
    }
    const otpData = otpDoc.data();
    // Check lockout (max 3 attempts)
    if (otpData.attempts >= 3) {
        throw new https_1.HttpsError('resource-exhausted', 'Maximum verification attempts exceeded. Please request a new OTP.');
    }
    // Check TTL (10 minutes)
    const tenMinutesMs = 10 * 60 * 1000;
    if (Date.now() - otpData.createdAt > tenMinutesMs) {
        await otpRef.delete();
        throw new https_1.HttpsError('deadline-exceeded', 'The verification code has expired. Please request a new one.');
    }
    // Verify OTP
    if (otpData.code !== otp) {
        // Increment attempts
    ...
    try {
        ...
        if (e.code === 'auth/user-not-found') {
            ...
            throw new https_1.HttpsError('internal', e.message || 'Authentication error.');
        }
    }
 ...
 */
exports.onRegistrationUpdate = (0, firestore_1.onDocumentUpdated)('registrations/{regId}', async (event) => {
 ...
 */
function generatePremiumCertificatePdf(studentName, eventName) {
